...
bool SyntaxChecker::visit(Literal const& _literal)
{
	...
	if (_literal.value().find("__") != ASTString::npos)
	{
	...
	{
		if (_literal.value().find("_.") != ASTString::npos)
		...
		if (_literal.value().find("_e") != ASTString::npos)
		...
		// No special validation on grouped decimals yet.
		return true;
	}
	...
	if (parts.front().size() > 4)
		m_errorReporter.syntaxError(_literal.location(), "Invalid use of underscores in hex literal. The first part " + parts[0] + " is too large. Group up by thousands.");
	...
	else if (parts.back().size() % 2 != 0)
		// First part is 4 digits, therefore last part's length has to be even to avoid ambiguity over zero padding.
